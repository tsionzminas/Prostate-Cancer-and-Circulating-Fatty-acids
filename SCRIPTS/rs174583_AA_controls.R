...
infile = file.path(PROJECT_DIR,"RESULTS","rs174583_AA_controls.txt")
data = as.matrix(read.table(infile,header=T,check.names=F,stringsAsFactors=F,sep="\t"))

# analysis_of_variance
explanatory_vars = c("rs174583")
param = colnames(data)[2:25]
...
outfile = file.path(PROJECT_DIR,"RESULTS","rs174583_AA_controls.pdf")
...
title(main="variance explained by rs174583 in AA controls",cex.main=1,line=1)
#title(main=paste0("[",measure_title[i_measure],"]"),cex.main=0.85,line=1)
axis(side=1,at=x,labels=param,las=2,cex.axis=0.45) # tck=-0.005
anova_txt = c(explanatory_vars,"residual")
anova_col = c("red","grey")
anova_pch = c(16,1)
for (i in 1:length(anova_txt)) {
  points(x,anova_res[,i],cex=1,col=anova_col[i],lty=2,type="b",lwd=2,pch=anova_pch[i])
}
x_legend = xrange[1] + (xrange[2]-xrange[1])*0.8
y_legend = yrange[1] + (yrange[2]-yrange[1])*0.75
